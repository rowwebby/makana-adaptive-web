...
import { ErrorCode, ConversationConfigurationError } from './errors';
import logger from './logger';

export interface WebStorageParams {
    organizationId: string;
    deploymentDeveloperName: string;
    messagingUrl: string;
}
...
export const isLocalStorageAvailable = () => {
    ...
    try {
        ...
        logger.info("localStorage not available.");
        return false;
    }
...
export const determineStorageType = (inLocalStorage = false) => {
    if (isLocalStorageAvailable() && inLocalStorage) {
        return localStorage;
 ...
 */
export const getItemInPayloadByKey = (payload: string | null, key: string) => {
    let item;
 ...
 */
export const initializeWebStorage = ({ organizationId, deploymentDeveloperName, messagingUrl }: WebStorageParams) => {
    storageKey = `${APP_CONSTANTS.WEB_STORAGE_KEY}_${organizationId}_${deploymentDeveloperName}`;

    const storage = determineStorageType();

    // existing web storage key found, do nothing
 ...
 */
export const getItemInWebStorageByKey = (key: string, inLocalStorage = false): string | undefined => {
    let item;
 ...
 */
export const setItemInWebStorage = (key: string, value: string, inLocalStorage = true) => {
    const storage = determineStorageType(inLocalStorage);
 ...
 */
export const removeItemInWebStorage = (key: string) => {
