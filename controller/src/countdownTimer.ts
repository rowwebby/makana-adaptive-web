...
export default class CountdownTimer {
    _timer: number | null | undefined; // Identifier returned when setTimeout() executes.
    _finished: boolean; // Whether the timer finished executing.
    _endTimerCallback: () => void; // Callback method after timer ends.
    _durationInMillis: number; // Time left (ms) on the timer.
    _lastReceivedTimestamp: number; // Timestamp of the most recent event.

    constructor(endTimerCallback: () => void, durationInMillis: number, timestamp: number) {
        this._timer = null;
     ...
     */
    reset(currentEventTimestamp: number) {
